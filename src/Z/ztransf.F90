...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  PURE REAL(KIND=DWP) FUNCTION ABODNZ2(B, M, X, Y, P, Q)
    ! skipping the diagonal elements X(P) and Y(Q),
    ...
    ABODNZ2 = D_ZERO
    !DIR$ FMA
    ABODNZ2 = ABODNZ2 + REAL(Y(P)) * REAL(Y(P))
    !DIR$ FMA
    ABODNZ2 = ABODNZ2 + AIMAG(Y(P)) * AIMAG(Y(P))
    !DIR$ FMA
    ABODNZ2 = ABODNZ2 + REAL(X(Q)) * REAL(X(Q))
    !DIR$ FMA
    ABODNZ2 = ABODNZ2 + AIMAG(X(Q)) * AIMAG(X(Q))
    ...
    IF (ABS(B(1,1)) .GE. ABS(B(2,1))) THEN
       ...
       IF (ABS(B(2,2)) .GE. ABS(B(1,2))) THEN
          ...
          DO I = 1, P-1
             ...
             AYY = ABS(YY)
             !DIR$ FMA
             ABODNZ2 = ABODNZ2 + (AXI - AXX) * (AXI + AXX)
             !DIR$ FMA
             ABODNZ2 = ABODNZ2 + (AYI - AYY) * (AYI + AYY)
          END DO
          !DIR$ VECTOR ALWAYS
          DO I = P+1, Q-1
             !DIR$ FMA
