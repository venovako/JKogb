...
CONTAINS
  ...
  PURE FUNCTION S_QUIET_NAN(PAYLOAD)
    USE, INTRINSIC :: ISO_C_BINDING
    IMPLICIT NONE
    INTEGER(KIND=c_int), PARAMETER :: MASK = 2143289344_c_int
    INTEGER(KIND=c_int), INTENT(IN) :: PAYLOAD
    REAL(KIND=c_float) :: S_QUIET_NAN
    S_QUIET_NAN = TRANSFER(IOR(PAYLOAD, MASK), 0.0_c_float)
  END FUNCTION S_QUIET_NAN

  PURE FUNCTION D_QUIET_NAN(PAYLOAD)
    USE, INTRINSIC :: ISO_C_BINDING
    IMPLICIT NONE
    INTEGER, PARAMETER :: K = MAX(c_long, IWP)
    INTEGER(KIND=K), PARAMETER :: MASK = 9221120237041090560_K
    INTEGER(KIND=K), INTENT(IN) :: PAYLOAD
    REAL(KIND=c_double) :: D_QUIET_NAN
    D_QUIET_NAN = TRANSFER(IOR(PAYLOAD, MASK), 0.0_c_double)
