...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE DSTEP_LOOP(N, U, LDU, A, LDA, Z, LDZ, J, SIGMA, NN, P, Q, R, NM, DZ, N_2, STEP, TT, W, INFO)
    ...
    INFO = 0
#ifdef ANIMATE
    CTX = PVN_RVIS_START(NF, NF, ACT, GNAME)
    ...
    DO WHILE (S .GE. 0)
#ifdef ANIMATE
       NF = PVN_RVIS_FRAME(CTX, A, LDF)
       IF (NF .NE. 0_c_int) THEN
          WRITE (ERROR_UNIT,'(A,I11)') 'PVN_RVIS_FRAME:', NF
          FLUSH(ERROR_UNIT)
          RETURN
       END IF
#endif
...
#ifdef ANIMATE
    NF = PVN_RVIS_STOP(CTX, SX, SY, BPP, GNAME)
    IF (NF .NE. 0_c_int) THEN
       WRITE (ERROR_UNIT,'(A,I11)') 'PVN_RVIS_STOP:', NF
       FLUSH(ERROR_UNIT)
    END IF
    CTX = 0_c_intptr_t
#endif
